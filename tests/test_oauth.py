...
def test_oauth_flow_reuse_authorization(oauth_app, login, gh_blogger):
    app_name = oauth_app.client_metadata["client_name"]
    authorized_url = 'http://localhost:33205/oauth/authorized'
    ...
    with login(gh_blogger) as client:
        ...
        # get a token with the authorization code
        token_resp = client.post('/oauth/token', data={
            'grant_type': 'authorization_code',
            'client_id': oauth_app.client_id,
            'code': query['code'],
        ...
        # try to reuse the authorization code to get a second token
        token_resp = client.post('/oauth/token', data={
            'grant_type': 'authorization_code',
            'client_id': oauth_app.client_id,
            'code': query['code'],
        ...
        assert token_resp.status_code == 400, token_resp.data
        assert b'Invalid \\"code\\"' in token_resp.data
